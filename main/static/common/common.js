...
function getSelectedRows() {
    ...
    //创建websocket建立消息提醒
/*	var ws = new WebSocket("ws://"+window.location.host+"/websocket/webSocketServer")
	ws.onopen = function () {
		console.log("onpen");
		//ws.send("{}");
...
function initProvince(){
	$.ajax({
		...
		success:function(data){
			var options = '<option code="" value="">'+opt0[0]+'</option>';
			var selected = "";
...
function initCity(father){
	if((typeof father) != 'undefined'){
		$.ajax({
			...
			dataType:"json",
			success:function(data){
				var options = '<option code="" value="">'+opt0[1]+'</option>';
				var selected = "";
				for(var i=0;i<data.data.length;i++){
...
function initArea(father){
	if((typeof father) != 'undefined'){
		$.ajax({
			...
			dataType:"json",
			success:function(data){
				var options = '<option code="" value="">'+opt0[2]+'</option>';
				var selected = "";
				for(var i=0;i<data.data.length;i++){
...
function initFileInput(fileId,inpId,uploadUrl){
	...
	var fileInputUrl = $("#"+inpId).attr("remote_url");
 ...
 * 初始化常规上传框
 */
function initUpload(fileid,imgid,inpid){
...
function initMulitUpload(fileid,imgdivid,inpid,maximg){
	...
	$("body").on("change","#"+fileid,function(){
		...
		var len = $("#"+imgdivid).find(".price-img").length;
		if(len>=maximg){
...
function initExcelUpload(fileid,url){
	...
	var shadeHtml = "<div class='modal fade my-shade' style='z-index:1099' aria-hidden='true' data-backdrop='static'>";
